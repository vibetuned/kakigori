...
def _check_isolated(args):
    ...
    p = multiprocessing.Process(target=_check_isolated_func, args=(q, mxl_path, target_classes))
    p.start()
...
def main():
    ...
    logger.info(f"Found {len(mxl_files)} MXL files to process. Target: {args.num_files} files.")
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    if is_match:
                        ...
                        if dest_path.exists():
                            dest_path = output_path / f"{mxl_path.stem}_{mxl_path.parent.name}.mxl"
    ...
    logger.info(f"Finished. Checked {checked_count} files, copied {copied_count} files to {output_path}")
