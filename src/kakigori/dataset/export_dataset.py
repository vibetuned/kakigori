...
def _convert_isolated(args):
    ...
    for attempt in range(3):
        ...
        q = ctx.Queue()
        p = ctx.Process(target=_convert_target_func, args=(q, mxl_path, mei_dir, krn_dir))
        p.start()
...
def main():
    parser = argparse.ArgumentParser(description="Convert MXL files to MEI and Humdrum files.")
    parser.add_argument("input_dir", type=str, help="Directory containing .mxl files")
    parser.add_argument("--mei-dir", "--mei_dir", dest="mei_dir", type=str, default="data/output_mei", help="Output directory for MEI files")
    parser.add_argument("--krn-dir", "--krn_dir", dest="krn_dir", type=str, default="data/output_krn", help="Output directory for Humdrum files")
    ...
    with ThreadPoolExecutor(max_workers=multiprocessing.cpu_count()) as executor:
        ...
        with tqdm(total=len(mxl_files), desc="Converting MXL", unit="file") as pbar:
            for future in as_completed(futures):
                ...
                pbar.set_postfix(
                    mei_ok=mei_success_count, mei_err=mei_error_count, 
                    krn_ok=krn_success_count, krn_err=krn_error_count
                )
                pbar.update(1)

    logger.info(f"Finished converting. MEI (Success: {mei_success_count}, Failed: {mei_error_count}) | Humdrum (Success: {krn_success_count}, Failed: {krn_error_count})")
