...
class GraphVisualizer(QMainWindow):
            ...
            try:
                ...
                for node_id in node_ids:
                    if node_id in node_map:
                        ...
                        rect_item = self.scene.addRect(QRectF(x1, y1, x2 - x1, y2 - y1), pen_node)
                ...
                for i in range(edge_index.shape[1]):
                    ...
                    if u_idx < len(node_ids) and v_idx < len(node_ids):
                        ...
                        if u_id in node_map and v_id in node_map:
                            ...
                            layer_name = EDGE_CLASSES.get(edge_type, f"Unknown Type {edge_type}")
                            
                            pen_edge = QPen(color)
                            pen_edge.setWidth(max(2, int(pixmap.width() * 0.0015)))
                            
                            line_item = self.scene.addLine(QLineF(ucx, ucy, vcx, vcy), pen_edge)
    ...
    def update_visibility(self):
        visible_layers = {layer for layer, cb in self.layer_checkboxes.items() if cb.isChecked()}
...
def main():
    ...
    parser.add_argument("--img-dir", "--img_dir", dest="img_dir", type=str, default="data/output_imgs")
    parser.add_argument("--ann-dir", "--ann_dir", dest="ann_dir", type=str, default="data/output_annotations")
    parser.add_argument("--graph-dir", "--graph_dir", dest="graph_dir", type=str, default="data/output_graphs")
