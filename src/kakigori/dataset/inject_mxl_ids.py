...
def inject_mxl_file(mxl_path: Path, output_dir: Path) -> bool:
    ...
    try:
        ...
        with zipfile.ZipFile(mxl_path, 'r') as zf:
            ...
            xml_names = [n for n in zf.namelist() if (n.endswith('.xml') or n.endswith('.musicxml')) and 'META' not in n]
            if not xml_names:
            ...
            meta_data = zf.read('META-INF/container.xml') if 'META-INF/container.xml' in zf.namelist() else None
...
def main():
    parser = argparse.ArgumentParser(description="Inject unique IDs into MXL elements to ensure deterministic tracking.")
    parser.add_argument("input_dir", type=str, help="Directory containing original .mxl files")
    parser.add_argument("--out-dir", "--out_dir", dest="out_dir", type=str, required=True, help="Output directory for processed .mxl files")
    ...
    with ProcessPoolExecutor(max_workers=multiprocessing.cpu_count()) as executor:
        futures = {executor.submit(inject_mxl_file, path, out_dir): path for path in mxl_files}
    ...
    logger.info(f"Finished processing. Success: {success_count} | Failed: {error_count}")
