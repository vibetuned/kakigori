...
class BBoxVisualizer(QMainWindow):
    ...
    def setup_ui(self):
        ...
        for group_name, classes in self.hierarchy.items():
            ...
            group_cb.toggled.connect(
                lambda checked, cb=group_cb, cls_list=classes: self.toggle_group(
                    checked, cls_list, cb
                )
            )
    ...
    def load_image(self):
        ...
        for ann in annotations:
            ...
            pen = QPen(color)
            pen.setWidth(
                max(1, int(pixmap.width() * 0.002))
            )  # Scale line thickness dynamically
            ...
            self.drawn_annotations.append(
                {"class": cls, "items": [rect_item, text_item]}
            )
