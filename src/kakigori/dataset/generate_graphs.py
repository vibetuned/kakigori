...
def generate_graph(
    mei_path: Path, json_paths: list, output_dir: Path, node_roles: dict
) -> bool:
    ...
    try:
        ...
        builder = GroundTruthGraphBuilder(
            str(mei_path), [str(j) for j in json_paths], node_roles
        )
...
def _generate_isolated(args):
    ...
    for attempt in range(3):
        ...
        q = ctx.Queue()
        p = ctx.Process(
            target=_generate_target_func,
            args=(q, mei_path, json_paths, output_dir, node_roles),
        )
        p.start()
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate PyG graphs from MEI and JSON annotations."
    )
    parser.add_argument("mei_dir", type=str, help="Directory containing .mei files")
    parser.add_argument(
        "json_dir", type=str, help="Directory containing annotation .json files"
    )
    parser.add_argument(
        "--out-dir",
        "--out_dir",
    ...
    parser.add_argument(
        ...
        dest="roles_file",
        type=str,
        default="conf/structure.json",
        help="Path to JSON file containing node roles",
    )
    ...
    logger.info(
        f"Finished generating graphs. Success: {success_count} | Failed: {error_count}"
    )
