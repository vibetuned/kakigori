...
class FlowMatchingUNet(nn.Module):
    def __init__(self, in_channels=3, base_dim=64):
        ...
        self.up_block1 = UNetBlock(
            base_dim * 4, base_dim * 2, time_dim
        )  # *4 because of skip connection

        self.up2 = nn.ConvTranspose2d(base_dim * 2, base_dim, 2, stride=2)
        self.up_block2 = UNetBlock(
            base_dim * 2, base_dim, time_dim
...
class FlowMatchingDiT(nn.Module):
    ...
    def forward(self, x, t):
        ...
        assert H == self.input_size and W == self.input_size, (
            f"Input size must be {self.input_size}"
        )
