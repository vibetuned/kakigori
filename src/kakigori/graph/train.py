...
def slice_page_to_systems(full_image_tensor, gt_boxes, gt_labels, gt_edges, class_to_idx):
    """
    ...
    for sys_box in system_boxes:
        ...
        current_sys_mapping = {old.item(): new for new, old in enumerate(original_indices)}
        ...
        system_datasets.append({
            ...
            'boxes': sys_boxes,
            'labels': sys_labels,
            'edges': torch.tensor(sys_gt_edges, dtype=torch.long) if sys_gt_edges else torch.empty((0, 3), dtype=torch.long)
        })
        
    return system_datasets

def train_phase3_end_to_end(detector, roi_extractor, gnn, full_page_dataloader, epochs=20):
    ...
    for epoch in range(epochs):
        ...
        for batch in full_page_dataloader: 
            ...
            system_groups = split_into_systems(abs_boxes, labels, gt_edges, class_to_idx)
            ...
            for sys_data in system_groups:
                ...
                roi_feats = roi_extractor(feat_maps, [sys_abs_boxes], full_image.shape[-2:])
...
def split_into_systems(abs_boxes, labels, gt_edges, class_to_idx):
    ...
    for sys_box in system_boxes:
        ...
        sys_gt_tensor = torch.tensor(sys_gt_targets, dtype=torch.long, device=abs_boxes.device) if sys_gt_targets else torch.empty((0, 3), dtype=torch.long, device=abs_boxes.device)
