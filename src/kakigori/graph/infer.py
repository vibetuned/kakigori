...
class FullPageOMRPipeline:
    ...
    def process_page(self, image_path, node_roles):
        ...
        image_tensor = TF.to_tensor(image).unsqueeze(0).to(self.device) # Shape: (1, C, H, W)
        ...
        for sys_idx, sys_box in enumerate(system_boxes):
            ...
            roi_features = self.roi_extractor(
                self.detector.extract_features(system_crop), # Assuming a feature extraction method
            ...
            edge_index = generate_axis_aware_edges(sys_primitives_boxes, sys_primitives_labels, self.class_to_idx)
            ...
            system_kern_string = generate_kern_stream(sorted_nodes, super_nodes, node_roles)
