...
class Music:
    ...
    def addBar(s, lbrk, m, balken, isSib):  # linebreak, measure data, staff allocations
        ...
        for v in s.vnums:
            ...
            s.voices[v] = sortMeasure(
                s.voices[v], m, balken, isSib, v
            ...
            if not s.nbr:
                mkBroken(s.voices[v])  # make broken rhythms
    ...
    def outVoices(s, maatrecs, ip, isSib):  # output all voices of part ip
        ...
        for iv in vnum_keys:
            if (
                s.cnt.getv("note", iv) == 0 and not s.wev
            ):  # no real notes counted in this voice
...
class ABCoutput:
    ...
    def mkHeader(
        s, stfmap, partlist, midimap, vmpdct, koppen, edo
...
def abcdur(nx, divs, uL):  # convert an musicXML duration d to abc units with L:1/uL
    ...
    if (
        nx.grace
    ):  # nx.type = whole, half, quarter, .. == 1,2,4, .. for {AA} and 2,4,8, .. for {A}
...
def outVoice(
    measure, divs, im, ip, unitL, rebeam, metrum
    ...
    for nx in measure:
        if isinstance(nx, Note):
            ...
            if (
                rebeam and nx.dur < divs and durvor < divs
            ):  # alleen tussen korte noten kijken
                nospace = tel == telvor  # noten binnen een tel
                if (
                    not nospace and nx.tijd % noemer != 0
                ):  # eerste noot in de volgende tel
...
def elemToNewVoice(nx, balken, balkvce, lastStaffChange, v, vtijd):
    ...
    if nx.xmlstf > 0 and nx.xmlstf != curstf:  # staff change needed
        lastStaffChange = (
            len(v),
            curstf,
        )  # (index in v of this change element e, current staff)
...
def sortMeasure(voice, m, balken, isSib, balkvce):
    ...
    for i, nx in enumerate(voice):  # establish sequentiality
        ...
        if isinstance(nx, Elem):
            if nx.tijd < vtijd:
                nx.tijd = vtijd  # shift elems without duration to where they fit
                ...
                if v[ixl].ns[0] == "z":  # shorten rest
                    ...
                    if v[ixl].dur == 0:
                        ...
                        if lastStaffChange:  # also delete possible staff change element
                            iv, cstf = (
                                lastStaffChange  # caused by the (now deleted) rest
                            )
                            del v[iv]
                            balken[balkvce] = cstf
                    info(
...
class Parser:
    ...
    def scanEdo(s, e):  # check for edo53
        ...
        for acc in accs:
            ...
            if a in s.edo53alt:
                edo53 = True
        if edo53 and edo36:
            info("this score mixes edo53 and edo36 accidentals.")
        if edo36 and s.temp == 1:
            edo53 = True
        ...
        if s.edo > 0:
            ...
            if sw and s.autoacc:
                if "abc2xml" in sw.lower():
                    s.musfin = 0
                if "burak" in sw.lower():
                    s.musfin = 1
                if "finale" in sw.lower():
                    s.musfin = 3
    ...
    def alt2acc(s, alt):
        ...
        if "." in alt:
            ...
            if s.musfin > 1:
                return None  # ignore microtonal alterations in finale/mscore
    ...
    def acc2abc(s, acc):
        ...
        if num == 0:
            ...
            if num > 0:
                ...
                acc = "_%d" % -num
            if s.temp == 2 and mapped:
                acc += "/3"  # fractional mapping
            if s.edo == 36:
                acc = acc.replace("3", "")  # because in edo36: ^3 == ^ and _3 == _
            return acc
        else:
            return ["__", "_", "=", "^", "^^"][num + 2]
    ...
    def ntAbc(s, ptc, oct, note, v, stf, ntrec, isTab):  # pitch, octave -> abc notation
        ...
        if acc != None:
            if s.musfin < 3:  # always show accidentals for finale
                if s.altkey:
                    ...
                    curalt = s.curalts.get((p, v, stf), None)  # only current alteration
            ...
            if s.musfin < 3:  # finale ignores alterations
                ...
                if curalt:  # no alt but previous note or the key had one -> natural!!
                    ...
                    if s.musfin == 1:
                        acc = curalt if s.ingrace else "natural"  # burak encoder
                    elif curalt in s.edo12alt:
                        acc = "natural"
                    elif s.musfin < 2:
                        acc = "natural"  # only honor microtonal alterations in strict mode
            # here we fall through with alt == None (and acc == None)
        else:  # alt has a value: map it
            if s.musfin < 3:
                acc = s.alt2acc(
                    alt
                )  # alt has a value: map it (finale ignores alterations)
        if acc != None:  # now see if we really must add an accidental
            ...
            info(
                "accidental %s added in part %d, measure %d, voice %d note %s"
    ...
    def doNote(s, n):  # parse a musicXML note tag
        ...
        grc = n.find("grace")
        note.grace = grc != None
        if note.grace and not chord:
            s.grcNotes.append(note)
        ...
        for clef in e.findall("clef"):  # a part can have multiple staves
            ...
            if first:
                ...
                for v in voices:
                    elem = Elem(
                        "[K:%s]" % cs, n
                    )  # n == staff of clef, for possible staff change in sortMeasure
    ...
    def doDirection(s, e, i, es):  # parse a musicXML direction tag
        ...
        if t != None:
            ...
            if scinst != None:
                ...
                chn, prg, vol, pan = s.instMid[s.msr.ixp][
                    instid
                ]  # midi values for the ID
                _, vs, _ = s.findVoice(
                    i, es
                )  # voice of the next note in the current measure
                if abcOut.volpan > 0:
                    s.msc.appendElem(vs, "[I:MIDI= %s %s]" % ("program", prg - 1))
    ...
    def emptyStaves(
        s, part, maten
    ):  # fill each empty staves with an invisible rest in a new voice
        def addRestToMsre(stf, vce, maat):
            ...
            xs = (
                '<note print-object="no"><rest measure="yes"/><duration>%s</duration>'
                % dvs
            )
            xs += "<voice>%d</voice><staff>%d</staff></note>" % (
                vce,
                stf,
            )  # new voice in empty staff
        ...
        estfs = [
            stf for stf, vlst in s.stfMap.items() if vlst == []
        ]  # all empty staves
    ...
    def parse(s, xmltxt):
        ...
        for ip, p in enumerate(parts):
            ...
            s.emptyStaves(
                p, maten
            )  # fill empty staves with new voices containing an invisible rest
            s.drumNotes = {}  # (xml voice, abc note) -> (midi note, note head)
            s.clefOct = {}  # xml staff number -> current clef-octave-change
            s.curClef = {}  # xml staff number -> current abc clef
            s.stemDir = {}  # xml voice number -> current stem direction
        ...
        if vvmapAll:  # skip output if no part has any notes
            abcOut.mkHeader(
                s.gStfMap, partlist, s.midiMap, s.tabVceMap, s.koppen, s.edo
            )
        else:
...
if __name__ == "__main__":
    ...
    parser.add_option(
        ...
        action="store",
        type="int",
        help="rules for microtonal accidentals (-1,0,1,2,3)",
        dest="fin",
        default=-1,
    )
    parser.add_option(
        ...
        action="store",
        type="int",
        help="temperament for microtonal accidentals (0,1,2)",
        dest="temp",
        default=0,
    )
    parser.add_option(
    ...
    parser.add_option(
        "--nbr",
        action="store_true",
        help="do not introdude broken rhythm",
        dest="nbr",
        default=False,
    )
    parser.add_option(
    ...
    for X, fnmext in enumerate(fnmext_list):
        ...
        if fnmext == "stdin.xml":
            ...
            for (
                ...
                if n[:4] != "META" and (
                    n.lower().endswith(".xml") or n.lower().endswith(".musicxml")
                ):
                    fobj = z.open(n)
