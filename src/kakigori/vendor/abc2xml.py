...
def pas(regex):  # regex => Gram (str => None | [str], str))
    def f(str):
        lengte = len(str)
        if G.zonderwit:
            str = str.lstrip()  # verwijder spaties voor het passen
...
class Gram:
    ...
    def parse(self, str):
        ...
        if r != None and self.transform:
            ...
            r = self.transform(r)
        if r == None:
            setLoc(self.beg)  # herstel de tekenindex na falen
        return r, rest
...
def abc_grammar():  # header, voice and lyrics grammar for ABC
    ...
    # -----------------------------------------------------------------
    b1 = pas("[-,'<>\u2019#]")  # catch misplaced chars in chords
    ...
    lyr_elem = (syllable | skip_note | extend_note | measure_end) + Suppress(
        Optional(white)
    )
    ...
    volta_nums = Suppress(Optional("[")) + Combine(
        digits + ZeroOrMore(oneOf(", -") + digits)
    )
    ...
    abc_voice = (
        ...
        + ZeroOrMore(Suppress(inline_field))
        + StringEnd()
    )

    abc_acclist = OneOrMore(
        Group(accidental + basenote)
    ).zonderWit()  # used in explicit key definition, like K:C ^F _1B _12/36G
    ...
    note_length.setParseAction(
        lambda t: [pObj("dur", (t[0], (t[2] << len(t[1])) >> 1))]
    )
    ...
    return (
        abc_header.zonderWit(),
        abc_voice.zonderWit(),
        abc_scoredef.zonderWit(),
        abc_percmap,
        abc_acclist,
    )
...
def detectBeamBreak(line, loc, t):
    ...
    xs = re.sub(
        r'"[^"]*"', "", xs
    )  # remove chords and annotations (because: may contain spaces)
...
def noteActn(t):  # detect beambreak between previous and current note/rest
    ...
    if debug:
        print("--noot", t.beg, t.end, repr(G.str[t.beg : t.end]))
    return [pObj("note", t)]
...
def restActn(t):  # detect beambreak between previous and current note/rest
    detectBeamBreak(G.str, t.beg, t)  # adds beambreak to parse result t as side effect
    if debug:
        print("--rust", t.beg, t.end, repr(G.str[t.beg : t.end]))
    return [pObj("rest", t)]
...
def errorWarn(t):
    ...
    loc = (
        t.end - 1
    )  # de fout kan met spaties beginnen en is dan een string met de letter aan het eind
    if loc > 40:
...
def doMaat(t):  # t is a Group() result -> the measure is in t[0]
    if debug:
        print("--maat", t.beg, t.end, repr(G.str[t.beg : t.end]))
...
def doGrace(t):  # t is a Group() result -> the grace sequence is in t[0]
    ...
    numNts = len(
        [nt for nt in grcNts if not getattr(nt, "chord", "")]
    )  # exclude chord notes
    for nt in grcNts:  # flag all notes within the grace sequence
        if nt.name == "note":
            nt.grace = numNts  # grace attribute = number of grace notes
...
def mergeMeasure(m1, m2, slur_offset, voice_offset, rOpt, is_grand=0, is_overlay=0):
    ...
    elif is_overlay and repbar != None:
...
class MusicXml:
    ...
    def reset(s, fOpt=False, mOpt=False):
        ...
        s.prevNote = [
            None,
            None,
        ]  # xml element of previous beamed note to correct beams (start, continue)
    ...
    def getAcc(s, acc):
        edo = 36 if acc.endswith("u") else s.edo
        acc = acc.replace("u", "")
        if acc in s.accTab[edo]:
            return s.accTab[edo][acc]
        if acc in s.accTab[12]:
            return s.accTab[12][acc]
        info(
            "error: unknown accidental %s in note: %s voice: %s, part: %s"
            % (acc, s.nid, s.vid, s.pid)
        )
        return "natural"
    ...
    def decTupAcc(s, tupacc):
        ...
        if acc in ["__", "^^"]:
            info(
                "error: a fractional suffix (%s) is ignored after %s, note: %s, voice %s, part %s"
                % (fraq, acc, s.nid, s.vid, s.pid)
            )
        elif "_" in fraq:
            ...
            if m == 53:
                ...
                acc = acc + str(n) + "u"  # markeer als up/down
            elif n == 1 and m == 1:
                pass
            elif n == 1 and m == 2:
                acc = acc + str(n)
                ...
                info(
                    "accidental %s%d/%d not supported, note: %s, voice %s, part %s"
                    % (acc, n, m, s.nid, s.vid, s.pid)
                )
        else:
            acc = acc + fraq
        return acc
    ...
    def mkPitch(s, acc, note, oct, lev):
        ...
        if (note, oct) in s.ties:
            ...
            if acc:
                s.msreAlts[(nUp, octnum)] = acc  # explicit notated alteration
            elif (nUp, octnum) in s.msreAlts:
                acc = s.msreAlts[(nUp, octnum)]  # temporary alteration
            elif nUp in s.keyAlts:
                acc = s.keyAlts[nUp]  # alteration implied by the key
    ...
    def mkNote(s, n, lev):
        ...
        if isgrace > 0:
            unitNum, unitDen = (
                (1, 16) if isgrace > 1 else (1, 8)
            )  # in ABC grace notes have fixed unit length
        ...
        if n.name == "rest":
            ...
            s.nid += step + (
                -oct * "," if oct < 0 else oct * "'"
            )  # current ABC note for error messages
            if type(acc) == tuple_type:
                acc = s.decTupAcc(acc)  # microtonal accidentals
            ...
            if s.mus53 and "." in alter:
                acc = acc53  # force accidentals everywhere!
    ...
    def doBeams(s, n, nt, den, lev):
        ...
        if hasattr(n, "chord"):
            return  # chord do not contain real beam breaks
        ...
        for ftype, field in fieldmap.items():
            ...
            if ftype == "Div":  # not an abc field, but handled as if
                ...
                if key:
                    ...
                    if fifths >= 0:
                        s.keyAlts = dict(zip(accs[:fifths], fifths * ["^"]))
                    else:
                        s.keyAlts = dict(zip(accs[fifths:], -fifths * ["_"]))
    ...
    def mkScorePart(s, id, vids_p, partAttr, lev):
        def mkInst(instId, vid, midchan, midprog, midnot, vol, pan, lev):
            ...
            try:
                ...
                imidprg = -1
            if imidprg < 0 or imidprg >= len(s.inst_tb):
                instnm = "no name"
            else:
                instnm = s.inst_tb[imidprg]
            if midchan == "10":
                iperc = int(midnot) - 27
                if iperc < 0 or iperc >= len(s.perc_tb):
                    instnm = "no name"
                else:
                    instnm = s.perc_tb[iperc]
            ...
            if imidprg >= 0:
                addElemT(
                    mi, "midi-program", str(imidprg + 1), lev + 2
    ...
    def doField_I(s, type, x, instDir, addTrans):
        ...
        def readPercMap(x):  # parse I:percmap <abc_note> <step> <MIDI> <notehead>
            ...
            p0, p1, p2, p3, p4 = abc_percmap.parseString(
                x
            )  # percmap, abc-note, display-step, midi, note-head
            acc, astep, aoct = p1
...
def writefile(pad, fnm, fnmNum, xmldoc, mxlOpt, tOpt=False):
    ...
    if pad:
        if not mxlOpt or mxlOpt in ["a", "add"]:
            ...
            if python3:
                outfile = open(outfnm, "w", encoding="utf-8")  # for Windows ...
            else:
...
abc_header, abc_voice, abc_scoredef, abc_percmap, abc_acclist = (
    abc_grammar()
...
def getXmlDocs(
    abc_string, skip=0, num=1, rOpt=False, bOpt=False, fOpt=False, mOpt=False
...
if __name__ == "__main__":
    ...
    parser.add_option(
        "--mus53",
        action="store_true",
        help="generate makam accidentals playable by musescore",
        default=False,
    )
    ...
    for fnmext in fnmext_list:
        ...
        skip, num = options.m
        xml_docs = getXmlDocs(
            abctext, skip, num, options.r, options.b, options.f, options.mus53
        )
