...
def preprocess(
    image: Image.Image, input_size: int, device: torch.device
...
def main():
    ...
    parser.add_argument(
        "--dpi", type=int, default=300, help="DPI for PDF page rendering."
    )
            ...
            detections = [
                {
                    ...
                    "bbox": [
                        round(
                            float(
                                ((bx1 - meta["pad_x"]) / meta["scale"]).clamp(min=0.0)
                            ),
                            1,
                        ),
                        round(
                            float(
                                ((by1 - meta["pad_y"]) / meta["scale"]).clamp(min=0.0)
                            ),
                            1,
                        ),
                        round(
