...
def get_checkpoint_step(checkpoint_dir):
    ...
    checkpoints = [
        int(c.name.split("-")[-1])
        for c in checkpoint_path.iterdir()
...
def format_ground_truth(targets_list, input_size, device):
    ...
    for tgt in targets_list:
        ...
        batch_targets.append({"boxes": abs_boxes, "labels": gt_labels})
...
def _evaluate_target_func(q, class_idx, class_name, preds, targets):
    ...
    try:
        ...
        q.put({"class_name": class_name, "map_50": 0.0, "map_75": 0.0, "map": 0.0})
...
def _evaluate_isolated(args):
    ...
    p = multiprocessing.Process(
        target=_evaluate_target_func, args=(q, class_idx, class_name, preds, targets)
    )
    p.start()
    ...
    if p.is_alive():
        ...
        logger.error(
            f" [!] Timeout: Class '{class_name}' calculation hung. Terminating process."
        )
        p.terminate()
        p.join()
...
def main():
    ...
    parser.add_argument(
        "--checkpoint", type=str, required=True, help="Path to checkpoint dir"
    )
    parser.add_argument("--img-dir", type=str, required=True, help="Validation images")
    parser.add_argument(
        "--ann-dir", type=str, required=True, help="Validation annotations"
    )
    ...
    # Tensorboard logging
    parser.add_argument(
        "--tb-dir", type=str, default="runs", help="TensorBoard log directory"
    )
    ...
    with torch.inference_mode():
        for batch in tqdm(val_loader, desc="Running GPU Inference"):
            ...
            preds = decode_model_outputs(
                outputs, args.conf_thresh, args.iou_thresh, args.input_size
            )
    ...
    for p, t in zip(all_preds, all_targets):
        for c in range(num_classes):
            ...
            class_data[c]["targets"].append(
                {"boxes": t["boxes"][t_mask], "labels": t["labels"][t_mask]}
            )
    ...
    with ThreadPoolExecutor(max_workers=num_cores) as executor:
        ...
        with tqdm(
            total=len(worker_args), desc="Computing mAP per class", unit="cls"
        ) as pbar:
    ...
    valid_classes = [
        res for res in class_results if res["map"] > 0 or res["map_50"] > 0
    ]

    global_map_50 = sum([r["map_50"] for r in valid_classes]) / max(
        1, len(valid_classes)
    )
    global_map_75 = sum([r["map_75"] for r in valid_classes]) / max(
        1, len(valid_classes)
    )
    ...
    if class_scores:
        ...
        for group_name, grouped_classes in hierarchy.items():
            tags = [
                f"mAP_Class/{cls_name}"
                for cls_name in grouped_classes
        ...
        uncategorized_tags = [
            f"mAP_Class/{cls_name}"
            for cls_name in class_scores
            if cls_name not in tracked_classes
        ]
        if uncategorized_tags:
            layout["Grouped_Class_mAP"]["Uncategorized"] = [
                "Multiline",
                uncategorized_tags,
            ]
